...
class Image {
     ...
     * @param  string $new_name   是新图片的名称
     * @param  int $width   缩放后的宽度
     * @param  int $height   缩放后的高度
     * @return mixed      是缩放后的图片名称,失败返回false;
     */
    function thumb($name, $new_name=null, $width=200, $height=100){
        if (is_null($new_name)) {
            $new_name = $name;
        }
        /* 获取图片宽度、高度、及类型信息 */
        ...
        return $this->createNewImage($newImg, $new_name,$imgInfo);
    }
    ...
    function base64EncodeImage ($name, $width=200, $height=100) {
        ...
        $newImg = $this->kidOfImage($srcImg, $size,$imgInfo);
        /* 通过本类的私有方法，保存缩略图并返回新缩略图的名称，以"th_"为前缀 */
        return $this->createBase64EncodeImage($newImg,$imgInfo);
    }
    ...
    function waterMark($groundName, $waterName, $waterPos=0, $qz="wa_"){
        /*获取水印图片是当前路径，还是指定了路径*/
        $curpath = rtrim($this->path,DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR;
        $dir = dirname($waterName);
        if($dir == "."){
            $wpath = $curpath;
        }else{
            $wpath = $dir.DIRECTORY_SEPARATOR;
    ...
    /* 内部使用的私有方法，用于保存图像，并保留原有图片格式 */
    private function createBase64EncodeImage($newImg, $imgInfo){
        ob_start();
        switch ($imgInfo["type"]) {
            case 1:       //gif
                imageGIF($newImg);
            ...
            case 3:       //png
                ...
                break;
            default:
                throw new Error('Picture format is not supported', 1);
                break;
        }
        $result = ob_get_clean();
        imagedestroy($newImg);
        return 'data:' . $imgInfo["mime"] . ';base64,' . chunk_split(base64_encode($result));
    }
